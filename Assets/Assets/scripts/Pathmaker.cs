...
//	Declare a public Transform called pathmakerSpherePrefab, assign the prefab in inspector; 		// you'll have to make a "pathmakerSphere" prefab later
	...
	void Update()
	{
		TileCounter = floorPrefab.childCount; 
...
//		If counter is less than 50, then:
		if (counter < 50)
		{
			...
			if (number <= .50f)
			{
				...
				else if (number >= .98f && number <= 1.0f)
				{
					Instantiate(pathmakerSpherePrefab, transform.position + new Vector3(0, 0, 0),
						Quaternion.Euler(new Vector2(1, 0)));
				}
		...
		Instantiate(floorPrefab, transform.position + new Vector3(0, 0, 0),
			Quaternion.Euler(new Vector2(1, 0)));
		{
			TileCounter++; 
		...
		if (counter >= 50)
		{
			Destroy(GameObject.Find("PathMakerSphere"));
			Destroy(GameObject.Find("pathmmakerSpherePrefab"));
		}
//		If counter is less than 50, then:
